  ...
  async runNow(): Promise<void> {
    ...
    try {
      ...
      if (result.scannedEvents > 0) {
        this.logger.log(
          `Replayed ${result.publishedEvents} checkout outbox events from ${result.scannedEvents} pending records`
        );
      }
    } catch (error: unknown) {
      const message =
        error instanceof Error ? error.message : 'Unknown error while replaying checkout outbox events';

      this.logger.error(
        `Failed to replay checkout outbox events: ${message}`,
