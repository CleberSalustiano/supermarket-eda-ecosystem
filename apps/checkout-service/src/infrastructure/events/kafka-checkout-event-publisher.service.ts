...
@Injectable()
export class KafkaCheckoutEventPublisherService
  implements CheckoutEventPublisherPort, OnApplicationShutdown
{
  ...
  async onApplicationShutdown(): Promise<void> {
    ...
    this.logger.log('Checkout Kafka producer disconnected');
  }
  ...
  private async ensureConnected(): Promise<void> {
    ...
    if (this.connectionPromise === undefined) {
      this.connectionPromise = this.producer
        ...
        .then(() => {
          this.connected = true;
          this.logger.log('Checkout Kafka producer connected');
        })
        .catch((error: unknown) => {
