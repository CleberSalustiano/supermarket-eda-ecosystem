...
@Injectable()
export class KafkaInventoryEventPublisherService
  implements InventoryEventPublisherPort, OnApplicationShutdown
{
  ...
  async onApplicationShutdown(): Promise<void> {
    ...
    this.logger.log('Inventory Kafka producer disconnected');
  }
  ...
  private async ensureConnected(): Promise<void> {
    ...
    if (this.connectionPromise === undefined) {
      this.connectionPromise = this.producer
        ...
        .then(() => {
          this.connected = true;
          this.logger.log('Inventory Kafka producer connected');
        })
        .catch((error: unknown) => {
